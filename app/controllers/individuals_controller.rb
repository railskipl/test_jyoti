class IndividualsController < ApplicationController
 ...
 def status
	...
	if params[:praise].present? && params[:criticism].present? || params[:praise].present? && params[:general].present? || params[:criticism].present? && params[:general].present?
			...
			if params[:praise]
				@praise = Praise.create(:email => params[:email], :praise_comment => params[:praise], :provider_user_id => params[:user_id],:typee => "praise", :city => current_user.city)
			end

			if params[:criticism]
				@criticism = Criticism.create(:email => params[:email], :criticism_comment => params[:criticism],:provider_user_id => params[:user_id], :typee => "criticism", :city => current_user.city)
			end

			if params[:helpful_tips]
				@general = General.create(:email => params[:email], :general_comment => params[:helpful_tips],:provider_user_id => params[:user_id], :typee => "general", :city => current_user.city)
			end
	...
	def facebook
		...
		params["to"].each do |reciver_id|
			@vote_track = VoteTrack.create(:user_id => current_user.id, :name => 'invitations')
		end
		if @inviteuser.present?
			a = params["to"].size
			b = a.to_i
			@inviteuser.first.invite_other = @inviteuser.first.invite_other + b
	...
	def indiv3
		if @status_check.vote_on_tips == true
			...
			@praise = Praise.where('provider_user_id != ? or provider_user_id IS ?', current_user.id, nil)
			@praises = @praise.where('email != ? and city != ? or city IS ?', current_user.email, current_user.city, nil)

			@criticism = Criticism.where('provider_user_id != ? or provider_user_id IS ?', current_user.id, nil)
			@criticisms = @criticism.where('email != ? and city != ? or city IS ?', current_user.email, current_user.city, nil)
      
			@general = General.where('provider_user_id != ? or provider_user_id IS ?', current_user.id, nil)
			@generals = @general.where('email != ? and city != ? or city IS ?', current_user.email, current_user.city, nil)
			
			#high priority 
			@second_priority1 = @praises.where('tip_viewed < ?', 3 ).order("RANDOM()").limit(10) rescue nil
			@second_priority2 = @criticisms.where('tip_viewed < ?',3 ).order("RANDOM()").limit(10) rescue nil
			@second_priority3 = @generals.where('tip_viewed < ?', 3 ).order("RANDOM()").limit(10) rescue nil
			b = []
